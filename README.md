...
- **각주처럼 번호가 붙는다** — 주석을 단 구간 뒤에 `[1]` 이 따라붙고, 그 번호는 아래 목록의 카드 번호와 같다.
  번호를 누르면 그 주석으로 간다. 번호는 표시일 뿐이라 파일에도, 복사한 글에도 들어가지 않는다.
...
| **이미지** | 붙여넣기(`Ctrl+V`), 드래그 앤 드롭. 넣은 그림은 노트와 같은 폴더의 `assets` 안에 파일로 남고, 마크다운에는 그 상대 경로가 적힌다 — 앱을 껐다 켜도, 폴더째 옮겨도 따라온다 |
| **붙여넣기** | 다른 곳에서 복사해 온 글은 글자색·배경색·글꼴을 떼고 이 노트의 서식으로 들어간다. 굵게·목록·제목 같은 뼈대는 그대로 온다 |
| **테마** | 라이트 / 다크 / 시스템 |
...
맥과 윈도우 모두 릴리스 파일을 실제 기기에 받아서 눌러 확인했다.
...
- [x] **윈도우 실기 확인** — 윈도우 11 에서 v0.2.0 설치 파일과 포터블 exe 를 받아 확인했다.
      폴더 열기, 주석(굵게를 가로지르는 구간·고아 주석 포함), 자동 저장, 새 노트/새 폴더,
      이름 바꾸기, 휴지통, 탐색기에서 보기, v0.1 설정 이전, 조용한 설치와 제거까지
...
- [ ] 위지윅으로 갔다 오면 코드펜스에 적어 둔 언어 이름(rust, js …)이 떨어지고,
      표 구분선과 문단 사이 빈 줄이 다시 써진다. 글자는 남지만 파일이 조금 달라진다
...
| **윈도우** — 설치 없이 | `Comment-Note_*_portable.exe` | 받은 파일을 그대로 실행한다 |

맥은 인텔과 애플 실리콘이 한 파일이다 (universal). 칩을 따질 필요가 없다.

윈도우 설치 파일은 현재 사용자에게만 설치한다. 관리자 권한을 묻지 않고
`%LOCALAPPDATA%\Comment Note` 에 들어가며, 설정은 `%APPDATA%\CommentNote\config.json` 에 남는다.
포터블 exe 는 그 옆에 `config.json` 이 있으면 그것을 먼저 쓴다 — USB 에 넣어 쓸 때를 위한 것이다.
